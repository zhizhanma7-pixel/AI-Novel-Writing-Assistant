...
- 正文写得和原计划不一致时，审阅面板会直接对照「原计划要求」与「正文实际写成」。
- 保留正文时可以同步调整后面章节的安排，也可以只记录这次变化、不动后续计划，两个选择是分开的。
- 需要参考时可以让 AI 给出后续章节的调整建议并说明理由，建议逐条采纳、采纳后仍可修改。
- 也可以把正文改回原计划；改写没成功时正文保持不变，这一条仍然可以继续处理。

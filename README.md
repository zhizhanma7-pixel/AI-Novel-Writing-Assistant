...
- 写法可以装成写法包（含 `SKILL.md` 的文件夹）在人之间拷来拷去：既能把别人的优秀文笔炼化进自己的库，也能把自己炼的拷给别人；导入的写法默认不自动生效，由作者决定何时开启
...
- 写法列表新增「导入写法包」：把一个含 `SKILL.md` 的文件夹拖进来，就能把别人炼化好的一套写法装进自己的库；导入前先看清名称、说明、四维规则篇幅和认不出的字段，包里带的脚本一律忽略且不会执行。
- 每条写法都能导出成一个 zip，解开就是可以直接拷给别人的写法包目录，不限于导入进来的那些；改过名称或规则后导出的是改后的版本，包里本项目不解读的附件和字段照旧带走。
- 写法可以按环节自动命中，写具体章节时不必每次手动绑定；导入的写法默认不参与自动命中，要在写法列表里自己打开。
- 提示词预览里能看出哪一条是自动命中带进来的、哪一条是自己绑定的，也能看出它为什么会出现在这一环节。
- 写法包声明了当前还没有对应环节的适用范围时，导入预览会直接说明它不会自动命中。
- 导入预览会提醒区分通用写作方法和具体剧情，并列出检测到的原作名称；损坏、异常膨胀或包含危险路径的 zip 会在导入前被拦住。
- 四维规则、自由说明和自定义小节会一起保留，自动命中会贯通正文写作、规划与审校。

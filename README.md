...
- 专业工作台新增变更提案面板，可查看 AI 建议的来源、风险和逐项差异，并逐项接受、修改或拒绝。
- 过期提案和并发修改会在写入前被拦截并给出恢复指引，部分批准也会明确询问其余项目如何处理。
- 自动导演等待提案审阅时会直接打开审阅面板；排队审批会显示处理状态，命令失败或等待超时时停止刷新并给出恢复建议。
- 行内修改会按正式执行内容保留数字等字段类型，界面提示与后端写入能力使用同一套规则。

...
- 自动导演触发安全熔断后按作品设定的问题处理规则真正执行：判「带警告继续」就接着写下一章，不再出现任务显示运行中却停着不动；判「暂停等人工」会重新排进恢复入口。放行后这一章若仍推进不了，任务会停在安全位置等你处理。
- 拆章完成后会重新投影角色动态，后续章节里角色的登场安排不会再沿用旧的拆章结果；这一步失败会自动排队重试，连重试都排不上时任务会停下来等你处理。

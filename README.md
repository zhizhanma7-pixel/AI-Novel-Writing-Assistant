...
- 新增「导入 SillyTavern 资产」页面：选一个导出文件即可，系统自己识别是角色卡、世界书还是预设。
- 角色卡不再整块变成一个角色，而是逐段确认去向：世界设定进知识库、文风存为写法资产、真正属于角色的才进角色。
- 世界书进知识库参与检索，重复导入不会产生重复条目；预设存为写法资产，其采样参数只作参考、不改变实际模型调用。

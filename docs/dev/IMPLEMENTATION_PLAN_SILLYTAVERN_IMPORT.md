...
| P12 | 未识别内容单独成段：默认不导入；作者选「世界设定」时原值原样进文档；选角色/文风被拒 | 真实 SQLite |
| P13 | 世界书顶层与条目内的未识别内容各自带值进文档，两者同时存在时都不丢 | 单测 |
...
| 编号 | 风险 | 处置 |
|---|---|---|
| R1 | 角色卡分流判错，把角色事实塞进世界观或反之 | 建议默认不采纳、逐条确认。原文的留存**有条件**：产生角色提案或写法资产时随之留存；内容全部去世界设定的那一趟两者都不产生，此时未识别内容作为可分流的一段交给作者决定是否附进世界文档 |

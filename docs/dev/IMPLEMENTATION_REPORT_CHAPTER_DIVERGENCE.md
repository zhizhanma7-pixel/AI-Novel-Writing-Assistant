...
> 当前状态：**Phase 2C 已合入 `beta`（`9db7142`）**；代码侧验证完成，**最终视觉验收仍待用户完成**
...
| G4：applier 直达越界回归 | `8266e37` | ✅ |
| 合入 `beta`（`--no-ff`） | `9db7142` | ✅ 69 文件 / +9079 行，无冲突 |
...
### 合入 `beta` 后的组合验证（2026-08-29）

`codex/chapter-divergence` 以 `--no-ff` 合入 `beta`（`9db7142`）。`beta` 此前停在
`2c5614f`、没有独有提交，因此合并基点即 `beta` 自身，无冲突。

在 `beta` 上干净重建 `shared` / `server` / `client` 后：

| 套件 | 结果 |
|---|---:|
| integration | **152 / 150 通过 / 2 跳过 / 0 失败** |
| fast | 1368 项，39 条既有失败，与合并前双向差集为空 |
| client | 190 项，4 条既有失败（移动端布局，与基线一致） |

另外做了一次运行时组合 smoke——这条比单跑套件更重要，因为 `beta` 上是
2A + 2B + 2C 的全部模块**一起加载**，而本阶段恰好修过两处模块循环加载崩溃：

| 检查 | 结果 |
|---|---|
| server 在 `beta` 代码上启动 | 正常 listening，无加载期异常 |
| 提案列表 / 详情 | 200，三条逐项，类型与「已修正」状态都对 |
| 建议端点、修正端点已注册 | 对不存在的逐项返回 404 `not_found`（路由挂上且错误映射正确） |
| 2B 卷规划仍可读 | 200 |

**注意本机的沙箱限制：** 每次工具调用在独立网络命名空间里，跨调用连不上自己
起的服务。上面这次 smoke 是在单个调用内完成启动与驱动的；要做界面验收必须由
用户在自己的终端起服务。
...
3. 两处待判断也有了结论：建议 prompt 声明
   ...
   次要入口，不新增端点。
4. **已合入 `beta`（`9db7142`），组合验证见上。**
5. **仍待办：最终视觉验收。** 代码侧只做了 typecheck、组件逻辑测试、三套套件回归
   与一次运行时 API smoke，**界面观感没有任何人看过**。要看的四点：两个接受出口、
   AI 建议、手动调整的越界提示、以及条目修正后操作是否消失。
6. 验收通过后再决定是否把 `beta` 晋升 `main`；本阶段不直接进 `main`。

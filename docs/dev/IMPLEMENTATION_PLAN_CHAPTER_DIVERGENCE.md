...
| 2C.4 接受分支 applier | ✅ T10/T11 真实 SQLite 1/1 | `79b4012` |
| 2C.5 修正分支接线 + 2C.6 G4 锁定 | ✅ 4/4 新增用例 + wiki | `ae02c5d` |
| 复审回填（H1 / H2 / H3 / M1–M5） | ✅ 见 `CODE_REVIEW_CHAPTER_DIVERGENCE.md` 5b–5e | `5bf5527`..`4065d2b` |
| H1 真实 producer → review → apply 组合测试 | ✅ 1/1 真实 SQLite | `7c07c5a` |
| 模块循环加载修复（顶层 eager 单例） | ✅ 由 H1/T1 的干净重建暴露 | `7088f77` |
| T1 端到端整书回归 | ✅ 1/1，整书跑完不被偏离提案中断 | `9389cf5` |
| 2C.7 界面 + 下游调整 + AI 建议 | ✅ 见 `IMPLEMENTATION_PLAN_CHAPTER_DIVERGENCE_UI.md` | `225a19c`..`5387eae` |

**Phase 2C 全部完成**（integration `148 / 146 通过 / 2 跳过 / 0 失败`）。
2C.7 的范围比本文原先那六行大：修正命令当时没有生产调用方，编辑期也没有校验，
K8 的下游补丁入口更是要单独设计。详见 UI 实施计划。

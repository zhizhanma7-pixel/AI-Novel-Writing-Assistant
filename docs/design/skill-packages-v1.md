...
说明保留，并在自动命中时按长度上限截断后带入。四维小节与自由说明、自定义小节可以
同时存在：导出会用当前四维规则替换已识别小节，但不会吞掉其余说明。
...
- **目前真的会触发的只有三个环节**：writer / planner / reviewer，映射到任务类型
  ...
  完整取值域，多出来的会在预览里被点名。
- **消费位置也必须贯通**：writer 进入正文上下文块；planner 进入规划期写法摘要；
  reviewer 同时进入检测与改写提示。只把匹配结果挂进运行时对象、不交给对应提示词，
  不算该环节已支持。
...
ZIP 只是传输容器，不是信任边界。浏览器在解压前后校验条目数、总展开体积、CRC、
重复路径、越界路径和加密标志；服务端解析器再次拒绝越界与重复路径，避免绕过界面
直调接口时把不安全路径存进资产。

## 导入预览的剧情作用域

写法包会跨作品复用，预览必须提示作者只保留写作方法、不要把某一本书的剧情事实当成
通用写法。检测到书名等源作实体时，预览会列出候选名称；它们会沿用生成期的确定性
消毒规则进入遮蔽清单。提示只帮助作者判断，不自动拒绝整个包，避免误判使可用写法无法导入。

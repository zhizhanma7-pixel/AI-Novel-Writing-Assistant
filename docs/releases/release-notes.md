...
### 2026-08-31（自动导演处置与投影修复）

- 自动导演触发安全熔断后，会按作品设定的问题处理规则真正执行：判「带警告继续」就接着写下一章，不再出现任务显示运行中却停着不动；判「暂停等人工」会重新排进恢复入口，而不是直接判任务失败。放行后如果这一章其实推进不了，任务会停在安全位置等你处理，不会反复重跑同一章。
- 后台执行中断后的恢复同样按作品规则处理，不再只记录一条问题、然后一律按默认方式恢复。
- 拆章完成后会重新投影角色动态，后续章节里角色的登场安排和卷内定位不会再沿用旧的拆章结果；这一步失败时会自动排队重试，不再只留一条日志。若连重试都排不上（重试次数已耗尽或队列不可用），任务会直接停下来进入恢复入口等你处理，而不是带着过期的角色投影继续往下写。
- 修复拆书并发生成角色时 token 用量少算的问题，预算上限现在能按实际消耗生效。
- 修复手机端「模型与厂商」等设置子页缺少适配样式的问题。

### 2026-08-30（导入 SillyTavern 资产）

- 新增「导入 SillyTavern 资产」页面（`/sillytavern-import`）：选一个从 SillyTavern 导出的文件即可，不用先分清它是角色卡、世界书还是预设，系统会自己识别并说明判断依据。
- 角色卡不再整块变成一个角色。卡里的内容会被拆开逐段确认去向：世界设定进知识库、文风要求存为写法资产、真正属于这个角色的才进角色。卡片自带的世界书会随世界设定一起导入。
- 角色描述和场景设定这两处最容易混着世界观，界面会标出来要求你逐段确认，不替你猜。放错的代价也写在选项里：世界设定放进角色，就只在这个角色身上生效。
- 世界书导入后进知识库并参与检索，原文里关掉的条目不会被导入；重复导入同一本不会产生重复条目。
- 预设导入后存为写法资产，原文里关掉的指令片段不会生效。预设里的采样参数会保留供查看，但**不会改变实际的模型调用参数**，模型与参数仍由模型设置决定。
- 支持角色卡 JSON、内嵌角色卡的 PNG、世界书 JSON 和预设 JSON；图片损坏、不是 PNG、图片里没有卡片数据等情况都会给出明确说明。
- 导入的世界设定要在知识库里绑定给作品，文风要在写法绑定里启用，之后才会生效。
- 本项目还不认识的字段会原样摆出来交给你决定：默认不导入，也可以选「世界设定」把原值附在世界文档末尾。世界书里顶层和条目内的未识别内容各自保留原值，不会只剩一个字段名。

### 2026-08-29（章节偏离审阅）

- 正文写得和原计划不一致时，审阅面板会直接对照「原计划要求」与「正文实际写成」，不再只显示一串字段路径。
- 保留正文时可以同步调整后面章节的安排，也可以只记录这次变化、不动后续计划。两个选择是分开的按钮，不会因为忘了填写而默认帮你决定。
- 需要参考时可以让 AI 给出后续章节的调整建议并说明理由，建议逐条采纳、采纳后仍可继续修改；后面的安排本来就成立时，AI 会直接说不用改。
- 也可以选择把正文改回原计划。改写没成功时正文保持不变，这一条仍然可以继续处理，不会被记成已经改好。
- 后续章节只开放本章目的、章末状态、下一章开场状态和独占事件四项调整，填写内容在保存时就会检查，不必等到批准后才发现无法执行。

### 2026-08-27（大纲导入与旧项目兼容性）
...
- 用户明确批准重大变化后可以正常执行，不会再次陷入重复审批；AI 也不能自行提高自动化权限、低报变化风险、提交与审阅展示不一致的实际写入值，或通过切换推进节奏绕过任务策略。
...
- 小说专业工作台新增变更提案面板，可以按状态和类型查找提案，查看建议依据、来源、风险与逐项前后差异。
- 每项变化都可选择接受、修改或拒绝；关系分值和角色状态支持直接改值，复杂变化可编辑完整内容，部分批准会明确询问其余项目的处理方式。
- 来源变化或多人同时审阅时，界面会在提交前阻止过期操作并给出中文恢复指引，不会自动重复批准或覆盖更新后的决定。
- 自动导演停在提案审阅点时，AI 驾驶舱和任务详情会直接打开审阅面板，不会把“继续推进”当成默认处理。
- 绑定导演任务的审批会显示等待处理状态并自动刷新结果；暂不能写入正式故事状态的变化会提前说明限制。
- 即使导演任务没有保存旧的恢复目标，提案检查点仍会稳定打开审阅面板；排队命令失败或等待超时时会停止刷新并给出明确处理建议，不再永久显示“等待导演处理”。
- 建议值缺失但正式执行内容已有数值时，行内修改会继续按数字保存；前后端共用同一字段映射与写入能力说明，减少界面与实际执行规则不一致。

### 2026-08-22（变更提案合并关口）

- 历史待确认记录中如果夹有已失效角色或格式错误的单项，系统会拒绝该项并继续处理其余合法变更，减少自动推进被一条旧脏数据反复卡住的情况。
- 经过正式审阅的整组提案继续保持原子执行；任何批准项无法安全写入时，整组都会停止并提示处理，不会只落下一半状态。
- 用户先保存逐项修改、稍后再审批时，可以直接确认使用已保存值，不必重复提交同一份修改内容。

### 2026-08-20（变更提案执行安全）

- 关系变更提案新增正式状态写入能力；用户修改 AI 建议值后，关系状态和当前关系阶段只会采用最终确认值。
- 对尚无正式状态写入能力的旧变更类型，执行会返回明确提示并保持提案未执行，避免账本显示成功但业务状态没有变化。
- 部分审批必须明确未列项目是接受还是拒绝，任务队列会完整保留这一选择，不再默认静默拒绝。
- 待审提案会让绑定的导演任务进入 `proposal_review_required` 检查点；审阅完成后任务状态按提案结果继续推进。
- 用户编辑过的提案在批准和执行后继续保留人工编辑来源与保护标记；旧的独立待审记录仍与新提案信封隔离。

### 2026-08-19（变更提案后端基础）

- 后端新增可版本化的变更提案与逐项审阅接口，支持整体批准、部分批准、编辑后批准、拒绝、再生和执行；当前阶段尚未提供提案审阅界面。
- 提案会记录用户可见依据、来源引用和风险等级；来源产物或章节内容变化后，过期提案会在批准或执行前被拦截。
- 多个审阅请求同时操作同一提案时，后到的逐项编辑会收到冲突提示，不会覆盖已经批准的内容。
- 正式状态写入继续使用现有状态提交、产物依赖和导演事件账本；执行只处理已批准或已修改批准的项目，拒绝项不会进入正式状态。
- PostgreSQL 与桌面 SQLite 数据库新增配对迁移，旧的独立待审变更保持兼容，并与新提案审阅链隔离。
